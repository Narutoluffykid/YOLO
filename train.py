...
from YOLO.utils.pascal_voc import pascal_voc
...
class Solver(object):
    ...
    def train(self):
        ...
        for step in range(1, self.max_iter + 1):
            ...
            load_timer.tic()    #载入数据前的系统时间
            images, labels = self.data.get()
            load_timer.toc()    #载入数据后的系统时间
            ...
            if step % self.summary_iter == 0:
                ...
                if step % (self.summary_iter * 10) == 0:
                    ...
                    train_timer.tic()   #开始训练前的时间
                    summary_str, loss, _ = self.sess.run(
                        [self.summary_op, self.net.total_loss, self.train_op],
                        feed_dict=feed_dict)    #训练数据
                    train_timer.toc()   #完成训练后的时间
...
def main():
    parser = argparse.ArgumentParser()  #用于编写用户命令接口的一个类
